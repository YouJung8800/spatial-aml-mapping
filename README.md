...
## 🛡️ 실무 도입을 위한 심화 파이프라인 (Data Imbalance & Label Scarcity 방어)

현업 결제 데이터의 두 가지 치명적 한계(정답 라벨 부재, 극단적 데이터 불균형)를 극복하기 위한 하이브리드 아키텍처입니다 (`advanced_aml_pipeline.py`).

### 핵심 기술 적용
1. **비지도 학습(Autoencoder) 기반 신뢰도 동적 할당:** 라벨이 없는 실 결제망에서 정상 패턴의 재건 오차(Reconstruction Error)를 역산하여 엣지(Edge)의 신뢰도를 모델 스스로 창출.
2. **Focal Loss 적용:** 정상 거래와 사기 거래의 극단적 불균형 환경에서 소수 클래스(이상 거래) 탐지에 가중치를 부여.
3. **PR-AUC 검증:** ROC-AUC가 가진 데이터 불균형 착시를 배제하고, 실무적인 사기 탐지(Fraud Detection) 성능을 객관적으로 증명.
